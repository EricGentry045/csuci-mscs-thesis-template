...
the deprived pool of Digital Forensic Investigators\cite{hitchcock2016tiered}
...
The \gls{seaker} device is a digital forensic triage tool that is designed to
be simple, portable, inexpensive, robust, and easy to use.  \gls{seaker} is an
acronym for Storage Evaluator And Knowledge Extraction Reader.  Utilizing a
\gls{rpi}, this digital forensic triage device is a novel approach to providing
...
of digital evidence.  \gls{seaker} is not meant to replace a fully-functional
digital forensic lab, but instead to augment the initial investigation and help
reduce the backlog.  This research and device overview proposes the mobile,
inexpensive, digital triage device called \gls{seaker}.
\end{abstract}
...
Collecting and analyzing evidence almost always requires a search warrant - a court-ordered search and
seizure of potential evidence of a location where a suspect resides, works, or may be storing it.
A search warrant is executed after it has been obtained from a judge and can involve physical and
...
A more field-friendly digital forensic {\em triage} solution like \gls{seaker} will assist in the initial
...
This paper presents a portable, inexpensive, efficient device, named \gls{seaker},
...
produced at the California State University Channel Islands (CSUCI)
campus in a 
...
\begin{enumerate}
  ...
  \item Co-authored a United States Department of Justice (DOJ) grant proposal, providing Logic Models
  for (\gls{seaker}) and Voyager (another similar digital forensic tool project at CSUCI)
\end{enumerate}
...
forensic examiners to follow: Computer Forensics Field Triage Process Model (CFFTPM).
The authors of the CFFTPM noted the important legal and technical
...
National Institute of Standards and Technologies (NIST) is a technological, non-regulatory
...
The NIST guidelines\cite{ayers2014guidelines} documenting a digital forensics process were published
...
The International Standards Organization (ISO) published a set of guidelines\cite{ISO27037}
...
ISO/IEC 27037 titled \uline{Guidelines for Identification, Collection, Acquisition, and
...
In addition, the United States Department of Justice (DOJ) first published 
...
{\em Integrated Digital Investigation Process} (IDIP)\cite{carrier2003getting},
proposed by Carrier, et al.  This was different from the DOJ proposal and
...
\begin{enumerate}
  ...
  evidence from the search warrant location.
...
based on the best parts of the US DOJ's \uline{Electronic Crime
...
on the success of the CFFTPM.  This amalgamation of approaches is still inspiring 
...
The ISO/IEC 27037 guidelines (completed in 2012) provide an attempt at an internationally 
recognized approach,
...
The \gls{seaker} device could be utilized to standardize on an approach to the triage phase of 
...
The second way \gls{seaker} will reduce digital forensics backlogs is by enabling a faster, more streamlined approach to initial
...
Within a few minutes of plugging in digital evidence media, \gls{seaker} will enable digital 
...
procedures.  These include training the digital forensic examiners and \gls{seaker} users, 
...
These are the main foci since the \gls{seaker}
...
warrant.  The \gls{seaker} device is intended to reduce the time this takes down to minutes,
especially during the triage stage of a search warrant execution.\\
...
Specifically, this research and the \gls{seaker} device focus on the triage stage of Analysis.  This
...
The \gls{seaker} device currently supports multiple operating systems, as well as multiple
device types (provided there is a way to adapt the device to USB). It achieves the goal of
being device and operating system agnostic in terms of being able to collect content
information from digital media. The only limitation to the \gls{seaker} device's ability
...
\begin{enumerate}
  ...
  \item Select {\em Search} based on default or custom search criteria
...
Digital forensic labs and TCUs are currently heavily inundated with cases needing
...
at an alarming rate [cite?], and has lead to a very large backlog of digital
...
The Internet of Things (IOT) also poses new challenges and could significantly
increase the backlog.  IOT devices are estimated to number near 40 billion by
...
potential uses of the \gls{seaker} device to include more digital media devices.
In addition, a portion of IOT devices are battery operated and computationally
...
or TCUs should be fully utilized.  The \gls{seaker} device is a great option for
...
(CFFTPM) is broken up into phases, two of which have sub-phases (see
...
based on type of investigation and prioritizing the investigation.  The \gls{seaker}
project combines the triage aquisition phase proposed here along with the analysis
phase, which is the rest of the CFFTPM on-scene process, into the full functionality
of the device.\\ 
...
when implemented on-scene at search warrant execution time.  The CFFTPM  proposed
...
With the proliferation of Internet Of Things (IOT) devices and cloud storage, the
...
the \gls{seaker} device, has the opportunity to expand it's influence on the initial
investigation phase by expanding to IOT and cloud storage devices.\\
...
Although useful, this is not the approach taken in the \gls{seaker} project.  Instead
of attempting to put together an alternate operating system that uses the potentially
tainted computer system, the \gls{seaker} device evaluates individual digital media
...
\begin{figure}[ht]
  ...
  \caption{iPhone WIFI connection to the \gls{seaker}.}
  \label{fig:screen-1}
  \end{center}
\end{figure}
