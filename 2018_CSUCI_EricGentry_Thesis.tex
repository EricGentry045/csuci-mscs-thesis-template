...
inexpensive, digital triage device called SEAKER.
\end{abstract}
...
Rogers et al\cite{rogers2006computer} (2006) seems to be among the first researchers to 
formally propose a traige phase in digital forensic investigations.  The process 
(\gls{cfftpm}) is broken up into phases (see
...
of cases.  Lillis et al\cite{lillis2016current} researched cloud
storage and found some areas of opportunity to expand the digital forensics
landscape, for instance parallel processing,
...
reduced I/O limitations in cloud storage.\\
...
{\em enhanced previewing}.  Enhanced previewing seeks to solve some of the problems
...
As is the case in other research, Shaw et
...
typically require advanced computer knowledge and a digital forensics specialist
to be involved.\\
...
The idea of a setup script for the \gls{rpi} was conceived by Eric Gentry 
...
Once Eric Gentry finished programming the setup script to properly configuring the
...
Eric Gentry chose the Apache web server, since it is a standard Unix-based operating system
choice for serving web pages.  It also supports backend coding opportunities when 
coupled with a server-side code execution program like PHP.  Setting this up also
done by Eric Gentry and was included as a part of the setup script.\\
...
The \textbackslash{}\verb|tmp| folder was chosen by Eric Gentry as a suitable location, since
...
lot of trial and error testing by Eric Gentry to make sure it all worked.\\
...
was performed by Eric Gentry that
...
Eric Gentry's theory on why this is the case is that the other programs have many
